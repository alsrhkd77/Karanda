...
class ChecklistNotifier with ChangeNotifier {
  ...
  bool get checklistItemIsNotEmpty => _checklistItem.values.any((element) => element.isNotEmpty);
  ...
  List<ChecklistItem> get weeklyMonCycleItem => _checklistItem[Cycle.weeklyMon] ?? [];
  List<ChecklistItem> get weeklyThuCycleItem => _checklistItem[Cycle.weeklyThu] ?? [];
  ...
  Future<void> setFinish(int index, ChecklistItem item, DateTime selected) async {
    try{
      ChecklistFinishedItem finishedItem = await _checklistItemProvider.createFinishedItem(item.title, selected);
  ...
  Future<void> removeFinish(int index, ChecklistItem checklistItem, int finishedItemIndex) async {
    bool result = await _checklistItemProvider.deleteFinishedItem(checklistItem.id!, checklistItem.finishedItem[finishedItemIndex].id!);
    if(result){
      _checklistItem[checklistItem.cycle]![index].finishedItem.removeAt(finishedItemIndex);
    }
    notifyListeners();
  }
  ...
  Future<void> createChecklistItem(ChecklistItem item) async {
    try{
      ChecklistItem result = await _checklistItemProvider.createChecklistItem(item);
  ...
  Future<void> updateChecklistItem(int index, Cycle cycle, ChecklistItem item) async {
    ChecklistItem? result = await _checklistItemProvider.updateChecklistItem(item);
    if(result != null){
